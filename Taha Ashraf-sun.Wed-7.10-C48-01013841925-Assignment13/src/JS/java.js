...
(function () {
    ...
    const Titleinput = document.getElementById('TaskTitile');
    const PriortyInput = document.getElementById('PriortyInput');
    const Dateinput = document.getElementById('Dateinput');
    const SecondSpinner = document.getElementById('Spinner');
    ...
    function getRelativeTime(createdTimestamp) {
        const now = Date.now();
    ...
    let ToDoArry = JSON.parse(localStorage.getItem('ToDoList') || '[]');
    const InProgressArry = JSON.parse(localStorage.getItem('InProgress') || '[]');
    const CompletedArry = JSON.parse(localStorage.getItem('CompletedList') || '[]');
    function ClearData() {
        Titleinput.value = '';
    ...
    function Display() {
        ...
        if (ToDoArry.length !== 0) {
            ...
            if (TodoCardContainer)
                TodoCardContainer.innerHTML = kartoona;
            checkForEmpty();
            UpdatedButtons();
            StartAccess();
            DeleteButtons();
    ...
    function DisplayInProgress() {
        ...
        if (InProgressArry.length !== 0) {
            ...
            if (InProgressContainer)
                InProgressContainer.innerHTML = kartoona;
            HideEmptyInProgress();
            UpdatedButtons();
            DeleteButtons();
            ToDOAccess();
    ...
    function DeleteButtons() {
        ...
        DeleteTrash.forEach((btn) => {
            btn.addEventListener('click', () => {
                ...
                const isInProgress = btn.closest('#InProgressContainer') !== null;
                const isCompleted = btn.closest('#CompleteContainer') !== null;
                let cat;
                if (isCompleted) {
                    cat = 'Completed';
                }
    ...
    function UpdatedButtons() {
        ...
        UpdatedElements.forEach((btn) => {
            btn.addEventListener('click', () => {
                ...
                let updatedElmnt = Number(btn.dataset.index);
                let taskStatus = btn.dataset.cat || 'ToDo';
